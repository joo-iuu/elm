    ...
    <ul class="addresslist">
      <li v-for="item in deliveryAddressArr" :key="item.daId">
        <div class="addresslist-left" @click="setDeliveryAddress(item)">
          <h3>{{ item.contactName }} {{ sexFilter(item) }} {{ item.contactTel }}</h3>
          <p>{{ item.address }}</p>
        </div>
...
export default {
  ...
  computed: {
    sexFilter() {
      return (item) => {
        return item?.contactSex == 1 ? '先生' : '女士';
      }
    }
  },
  methods: {
    ...
    setDeliveryAddress(deliveryAddress) {
      this.$setLocalStorage(this.user.userId, JSON.stringify(deliveryAddress));
      this.$router.push({ path: '/orders', query: { businessId: this.businessId, address: JSON.stringify(deliveryAddress) } });
    },
    toAddUserAddress() {
